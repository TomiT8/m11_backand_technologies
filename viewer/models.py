...
from django.db.models import Model, CharField, DateField, ForeignKey, SET_NULL, TextField, ManyToManyField, \
    DateTimeField, IntegerField, CASCADE, ImageField, FloatField, PositiveIntegerField
...
class Creator(Model):
    ...
    def age(self):
        if self.date_of_birth:
            ...
            if (end_date.month < self.date_of_birth.month) or (end_date.month == self.date_of_birth.month and end_date.day < self.date_of_birth.day):
                years -= 1
            return years
        return None
...
class Review(Model):
    ...
    def __repr__(self):
        return (f"Review(movie={self.movie}, review={self.reviewer}, "
